...
# This is for meta learning to enable N-way-K-shot learning
 
class ClusteredGroupClassSampler:
    r"""
        Samples batches of data from predefined groups.
    """

    def __init__(self, dataset, meta_batch_size, support_size, way_num, shot_num,
                 drop_last=None, uniform_over_groups=True, args=None):

        self.dataset = dataset
        self.indices = range(len(dataset))

        self.class_with_ids = dataset.class_with_ids
    ...
    def __iter__(self):
        ...
        if self.uniform_over_groups:
            sampled_groups = np.random.choice(self.groups, size=(n_batches, self.meta_batch_size // 2))
        else:
            # Sample groups according to the size of the group
            sampled_groups = np.random.choice(self.groups, size=(n_batches, self.meta_batch_size // 2), p=self.group_prob)
            
        # Sample labels: N-way
        sampled_classes = np.random.choice(self.classes, size=(n_batches, self.way_num))

        for batch_id in range(self.num_batches):
            train_sampled_ids = [np.random.choice(np.asarray(list(set(self.groups_with_ids[sampled_groups[batch_id, sub_batch]]).intersection(set(self.class_with_ids[sampled_classes[batch_id, sub_class]])))),
                                size=self.shot_num,
                                replace=True,
                                p=None)
                                for sub_class in range(self.way_num) for sub_batch in range(self.meta_batch_size // 2)]
            test_sampled_ids = [np.random.choice(np.asarray(list(set(self.groups_with_ids[sampled_groups[batch_id, sub_batch]]).intersection(set(self.class_with_ids[sampled_classes[batch_id, sub_class]])))),
                                size=self.shot_num,
                                replace=True,
                                p=None)
                                for sub_class in range(self.way_num) for sub_batch in range(self.meta_batch_size // 2)]
            ...
            # Flatten
            train_sampled_ids = np.concatenate(train_sampled_ids)
            test_sampled_ids = np.concatenate(test_sampled_ids)
            sampled_ids = np.concatenate([train_sampled_ids, test_sampled_ids])
...
#         if dataset.__class__.__name__ == 'imageNetDataset' or dataset.__class__.__name__ == 'rimageNetDataset':
#             self.num_groups = 75
#         else:
        self.num_groups = dataset.n_groups
...
#         if dataset.__class__.__name__ == 'imageNetDataset' or dataset.__class__.__name__ == 'rimageNetDataset':
#             self.group_ids_probs_pre = self.group_ids_one_hot * (1 / dataset.n_groups)
#         else:
        self.group_ids_probs_pre = self.group_ids_one_hot * (1 / self.group_count)
